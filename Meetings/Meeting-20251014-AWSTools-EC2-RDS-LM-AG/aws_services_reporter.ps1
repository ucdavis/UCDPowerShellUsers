...
Get-LMFunctionList -ProfileName engr-psdemo | Get-LMFunction -ProfileName engr-psdemo | Select-Object -Property FunctionName,Runtime,MemorySize,Timeout.CodeSize,LastModified,RoleName | Export-Csv -Path ("AWS_Report_Lambda_Functions_" + (Get-Date).ToString("yyyy-MM-dd-HH-mm") + ".csv") -NoTypeInformation;

#Display Lambda Functions
Get-LMFunctionList -ProfileName engr-psdemo | Get-LMFunction -ProfileName engr-psdemo | Select-Object -Property FunctionName,Runtime,MemorySize,Timeout.CodeSize,LastModified,RoleName | Format-Table -AutoSize
